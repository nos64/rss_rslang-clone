...
> **Работа с Axios**:
>
> - Добавил папку src/axios. Туда разместил инстанс axios, через который нужно делать http запросы. Импортируем `$api` и делаем нужные запросы. Так же для запросов вам > может понадобиться userId, он храниться в localStorage: `localStorage.getItem('userId')`.
>
> ```
